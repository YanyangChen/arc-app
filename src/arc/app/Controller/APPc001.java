...
//import arc.apw.Controller.APWc001.Search;
//import arc.apw.Controller.APWc001.Search2;

@Controller
@Scope("session")
@ACFgFunction(id="APPF001")
@RequestMapping(value=APPtMapping.APPF001)
public class APPc001 extends ACFaAppController {
  ...
  //  Search search = new Search();
    ...
    private class Search extends ACFgSearch {
        public Search() {
            super();
            setCustomSQL("select * from (select m.* from arc_item_master m where m.section_id = '04')");
            setKey("item_no");
...
//            setModel(ARCmItemMaster.class); //define a Search which accept 4 filters from client
           setCustomSQL("select * from (select ic.item_category_no from arc_item_category ic where section_id = '04') ");
...
//    Search search = new Search();
    ...
    @RequestMapping(value=APPtMapping.APPF001_MAIN, method=RequestMethod.GET)
    public String main(ModelMap model) throws Exception {
       ...
       // model.addAttribute("sub_section_id", sub_section_id); //set tow keys
        ...
        model.addAttribute("modules", ItemCategoryService.getCat_Noapp()); //acf's function, get data from ACFDB
    ...
    @RequestMapping(value=APPtMapping.APPF001_SEARCH_AJAX, method=RequestMethod.POST)
    @ResponseBody
    ...
    @RequestMapping(value=APPtMapping.APPF001_GET_FORM_AJAX, method=RequestMethod.POST)
    @ResponseBody
    public ACFgResponseParameters getForm(@RequestBody ACFgRequestParameters param) throws Exception {
        ...
        getInventory(param.getRequestParameter("inventory_browse"));//pending delivery
	...
	@RequestMapping(value=APPtMapping.APPF001_GET_REMAINING_INVENTORY_AJAX, method=RequestMethod.POST)
    @ResponseBody
     public ACFgResponseParameters getRemainingInventory(@RequestBody ACFgRequestParameters param) throws Exception {
      ...
      select.setCustomSQL("select * from (select I.item_no, I.purchase_order_no, I.purchase_order_date, I.unit_cost, I.received_quantity + I.adjusted_quantity - I.consumed_quantity "
              ...
              + "I.back_order_quantity, I.modified_at, IM.material_type, IM.material_type, "
              ...
              + "from arc_item_inventory I, arc_item_master IM "
              + "where I.item_no = IM.item_no)");
	...
	LinkedHashMap<String, Object> lhm = (LinkedHashMap<String, Object>) ar.getRows().get(i-1); //should get all the rows
      ...
      if ((!lhm.get("item_no").toString().substring(3,7).equals("9999")) && (lhm.get("material_type").equals("2")))
    	  
      {
    	  lhm.put("remaining_quantity", "0");
    	  lhm.put("unit_cost", "0");
      }
      
      
      if ((lhm.get("remaining_quantity").equals(0.00) || lhm.get("remaining_quantity").toString().equals("0.00") 
    		  || lhm.get("remaining_quantity").toString().equals("0")) )
      {
    ...
    @RequestMapping(value=APPtMapping.APPF001_GET_INVENTORY_AJAX, method=RequestMethod.POST)
    @ResponseBody
    ...
    @RequestMapping(value=APPtMapping.APPF001_ADJUSTMENT_AJAX, method=RequestMethod.POST)
    @ResponseBody
    ...
    @RequestMapping(value=APPtMapping.APPF001_GET_REFERENCE_PRICE, method=RequestMethod.POST)
    @ResponseBody 
    ...
    @ACFgTransaction
    @RequestMapping(value=APPtMapping.APPF001_SAVE_AJAX, method=RequestMethod.POST)
    @ResponseBody
...
//                    public void callback() throws Exception {
	                    ...
	                    public void callback() throws Exception {
							...
							if(newItem.item_no.substring(3,7).equals("9999")&&(!newItem.material_type.equals("2")))
							{
								//Item No. must be 7 digits!
								throw exceptionService.error("APP201E");
							}
...
//							Set<String> VALUES = new HashSet<String>(Cats);
                      ...
                      //      ItemInventoryDao.saveItems(ItemInvRemamendments);
                        ...
                        //if 9999 has not been created yet, create one first
                        if ((ItemInventoryDao.selectItem(newItem.item_no.substring(0,3)+"9999") == null) && (newItem.material_type.equals("2")))
                        {
////                       
                        	if (!newItem.item_no.equals(newItem.item_no.substring(0,3)+"9999") && (ItemMasterDao.selectItem(newItem.item_no.substring(0,3)+"9999") == null)){
                        	throw exceptionService.error("APP301E"); }  //Must create item '9999' first! 	
                        }
//                      	
...
//            		ARCmItemInventory II = ItemInventoryDao.selectItem(oldItem.item_no);
            	...
            	{throw exceptionService.error("APP003E");}
                return false;
            }
        });
